		...
		if(clowder != null) {
			...
			if(Clowder.getClowderFromName(name) == null) 
			{
				...
				if(clowder.getPermLevel(player.getDisplayName()) > 1) {
					...
					ArrayList<int[]> flagCoords = new ArrayList<int[]>(); 
					
					for(CoordPair a : ECommands.flagLocs.values()) {
						int y = ECommands.getHeightValue(sender.getEntityWorld(), a.x, a.z) + 1;
						TileEntity te = sender.getEntityWorld().getTileEntity(a.x, y, a.z);
						for(int i = 0; i < 2; i++)
							if(te == null) {
								y--; 
								te = sender.getEntityWorld().getTileEntity(a.x, y, a.z);
							}
						if(te != null && te instanceof TileEntityFlagBig) {
							TileEntityFlagBig flag = (TileEntityFlagBig)te;
							if(flag.owner == clowder)
								flagCoords.add(new int[] {flag.xCoord,flag.yCoord,flag.zCoord});
						}
					}
					
					clowder.rename(name, player);
					clowder.save(player.worldObj);
					for(int[] a : flagCoords) {
						player.worldObj.getChunkProvider().loadChunk(a[0]/16, a[2]/16);
						player.worldObj.getChunkProvider().loadChunk(a[0]/16, (a[2]+1)/16);
						TileEntity te = sender.getEntityWorld().getTileEntity(a[0], a[1], a[2]);
						if(te != null && te instanceof TileEntityFlagBig) {
							TileEntityFlagBig flag = (TileEntityFlagBig)te;
							if(flag.owner == null) {
								flag.owner = clowder;
				...
				if (clowder.getPermLevel(player.getDisplayName()) > 1) {
					...
					if (!Clowder.colours.contains(c)) {
						if (c < 0) {
							...
							for(Map.Entry<String, CoordPair> entry : ECommands.flagLocs.entrySet()) {
								int y = ECommands.getHeightValue(world, entry.getValue().x, entry.getValue().z);
								TileEntity te = world.getTileEntity(entry.getValue().x, y, entry.getValue().z);
