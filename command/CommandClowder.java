...
import com.LordWeeder.EconomyPlus.registry.ECommands;
...
//			}break;
			...
			else if (victim.isRaidable() )
			{
						 ...
						 if(attacker.getFabricatetime() == 0 && attacker.getWartime() == 0 && attacker.getCanDeclareTime() == 0) //for when the command successfully works
				{
					...
					sender.addChatMessage(new ChatComponentText(ERROR + "War declaration is already complete! Use /c declare to begin the war!"));
						...
						sender.addChatMessage(new ChatComponentText(ERROR + "Calm down, you have attempted to fabricate on seven factions within the past few hours. Take a break"));
				...
				sender.addChatMessage(new ChatComponentText(ERROR + "Your faction does not have enough prestige to handle its vast territories!"));
				...
				if(clowder.enemy != null)
				{
					...
					if (clowder.getCanDeclareTime()<=0 && clowder.getWartime() > 0)
					{
						...
						clowder.enemy.notifyAll(player.worldObj, new ChatComponentText(CommandClowder.TITLE + "They can only do this once per war, and it costed them triple prestige!"));
							...
							sender.addChatMessage(new ChatComponentText(ERROR + "Your faction has already used over-time during this war!"));
						}
						else
	...
	private void cmdHelp(ICommandSender sender, String page) {
		...
		if (p == 1) {
			sender.addChatMessage(new ChatComponentText(COMMAND_LEADER + "-fabricate <name>" + TITLE
					+ " - Fabricates a war against a faction, costs " + MainRegistry.fabricateCost + " prestige"));
			...
			*/
			sender.addChatMessage(new ChatComponentText(
					COMMAND_LEADER + "-suckoff <name>" + TITLE + " - Sends a tribute offer to a faction"));
			sender.addChatMessage(new ChatComponentText(
		...
		if (p == 2) {
			...
			sender.addChatMessage(new ChatComponentText(
					COMMAND_ADMIN + "-owner <player>" + TITLE + " - Transfers faction ownership"));
			sender.addChatMessage(
					new ChatComponentText(COMMAND + "-comrades" + TITLE + " - Shows all members of your faction"));
			sender.addChatMessage(new ChatComponentText(
					COMMAND_LEADER + "-color <hexadecimal>" + TITLE + " - Sets the faction's color"));
			sender.addChatMessage(
		...
		if (p == 3) {
			sender.addChatMessage(
					new ChatComponentText(COMMAND + "-info {page}" + TITLE + " - Shows info on a faction"));
			sender.addChatMessage(
			...
			sender.addChatMessage(new ChatComponentText(
					COMMAND_LEADER + "-kick <player>" + TITLE + " - Removes player from faction"));
		...
		if (p == 5) {
			sender.addChatMessage(new ChatComponentText(COMMAND + "-retreat" + TITLE
					+ " - Protects entire faction from enemy fabrications by retreating after 10 minutes"));
			sender.addChatMessage(new ChatComponentText(COMMAND + "-retreat" + TITLE
			...
			sender.addChatMessage(new ChatComponentText(
					COMMAND + "-balance" + TITLE + " - Displays how much prestige the faction has"));
			sender.addChatMessage(new ChatComponentText(
			...
			if (clowder.canDisband) {
				if (name.equals(clowder.name)) {
					...
					if (clowder.disbandClowder(player)) {
						...
						sender.addChatMessage(
								new ChatComponentText(ERROR + "Can not disband a faction you do not own!"));
					}

				} else {
					sender.addChatMessage(new ChatComponentText(ERROR
							+ "Confirmation unsuccessful. Please enter the faction name to disband the faction."));
				}
			} else {
				sender.addChatMessage(new ChatComponentText(
	...
	private void cmdRevolt(ICommandSender sender) {
		...
		sender.addChatMessage(new ChatComponentText(ERROR + "You need to be in a faction that is a vassal!"));
	}
			...
			if(clowder != null) {
				if(clowder.getPermLevel(player.getDisplayName()) > 1) 
				{
					...
					else if(victim.enemy == clowder && victim.getCanDeclareTime() > 0 && victim.getWartime() <= 0)
					{
						...
						sender.addChatMessage(new ChatComponentText(ERROR + "That faction is not enough of a threat!"));	
			...
			if(clowder != null) {
				if(clowder.getPermLevel(player.getDisplayName()) > 1) 
				{
					 ...
					 if(victim.enemy == clowder && victim.suzerain == clowder && victim.getFabricatetime() > 0 && victim.getWartime() <= 0)
					{
						...
						sender.addChatMessage(new ChatComponentText(ERROR + "That faction is either not your vassal or is not revolting!"));	
	...
	public static void cmdLeave(ICommandSender sender) {
		...
		if (clowder != null) {
			...
			if (clowder.getPermLevel(player.getDisplayName()) < 3) {
				...
				sender.addChatMessage(new ChatComponentText(ERROR + "Bitches can not leave a faction!"));
			}
			...
			if (clowder.getPermLevel(player.getDisplayName()) > 1) {
				...
				if (clowder.applications.contains(name)) {
					...
					if (Clowder.getClowderFromName(name) == null) {
						...
						sender.addChatMessage(
								new ChatComponentText(INFO + "Added player " + name + " to your faction!"));
	...
	private void cmdKick(ICommandSender sender, String kickee) {
		...
		if (clowder != null) {
			...
			if (!clowder.targeted) {
				...
				sender.addChatMessage(new ChatComponentText(
						ERROR + "Bitches cannot kick faction members!"));
						...
						if (te != null && te instanceof TileEntityFlagBig) {
							...
							if (insideBorders(new CoordPair(flag.xCoord / 16, flag.zCoord / 16))) {
								if (clowder.getPrestige() >= flag.getCost()) {
									...
									// Updating the region on the dynmap
									DynmapInterface iFace = new DynmapInterface(DynmapPlugin.plugin);
									iFace.updateArea(flag.provinceName, clowder.color, 0.8);
...
//								
//								//Setting the physical flag's colour to white
//								flag.updateGauge(ClowderFlag.NONE.ordinal(), 0, 100);
//								flag.updateGauge(0xFFFFFF, 1, 100);
//								
								// Updating the region on dynmap
								DynmapInterface iFace = new DynmapInterface(DynmapPlugin.plugin);
								iFace.updateArea(flag.provinceName, 0xffffff, 0.5);
			...
			if (clowder.getPermLevel(player.getDisplayName()) > 2) {
				...
				if (clowder.members.get(demotee) != null) {
					...
					if (clowder.getPermLevel(demotee) == 2) {
						...
						sender.addChatMessage(new ChatComponentText(
								ERROR + "Are you seriously trying to demote the faction's leader?"));
					}
